...
describe("stash", () => {
  ...
  it("handles prototype-less objects created with Object.create(null)", async () => {
    const obj = Object.create(null);
    obj.a = obj;
    const stashed = await stashAsync(obj);
    const unstashed = await unstashAsync(stashed);
  ...
  it("handles prototype-less objects with symbol keys", async () => {
...
describe("identical objects", () => {
  ...
  it("accepts a value in milliseconds for yieldThreadEvery", async () => {
    const data: any = {
      circular: [],
      regexes: [],
      maps: [],
    ...
    for (let i = 0; i < 10000; i++) {
      data.circular.push(data);
      data.regexes.push(/a/);
      data.maps.push(new Map([["self", data]]));
    }
    let start;

    start = performance.now();
    const stored40 = await stashAsync(data, [], 40);
    const elapsed40 = performance.now() - start;
    expect(stored40).toBeDefined();

    start = performance.now();
    const stored100 = await stashAsync(data, [], 100);
    const elapsed100 = performance.now() - start;
    expect(stored100).toBeDefined();

    console.log(elapsed40, elapsed100);
    expect(elapsed100).toBeLessThan(elapsed40);
  });
});
